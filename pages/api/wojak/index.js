...
const fetchPwi = async () => {
  try {
    ...
    console.error("Error occured when fetching Pink Wojak Index: ", err.message);
    return { error: err };
  }
};
...
const fetchTicker = async (symbol) => {
  try {
    ...
    return { tickerData: { lastPrice: Number(lastPrice), priceChangePercent: Number(priceChangePercent), symbol } };
  } catch (err) {
    console.error("Error occured when fetching ticker from Binance: ", err.message);
    return { error: err };
  }
};
